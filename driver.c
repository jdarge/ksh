...
static const char* const initcoms[] = {
        ...
        "type=whence -v",
#ifdef JOBS
        "stop=kill -STOP", "suspend=kill -STOP $$",
#endif
#ifdef KSH
        "autoload=typeset -fu", "functions=typeset -f",
# ifdef HISTORY
        "history=fc -l",
# endif /* HISTORY */
        "integer=typeset -i", "nohup=nohup ", "local=typeset", "r=fc -e -",
#endif /* KSH */
#ifdef KSH
...
int main (int argc, char* argv[])
{
    ...
    /* Figure out the current working directory and set $PWD */
    {
        ...
        if (!ISABSPATH(pwd) || stat(pwd, &s_pwd) < 0 || stat(".", &s_dot) < 0 ||
            s_pwd.st_dev != s_dot.st_dev || s_pwd.st_ino != s_dot.st_ino)
        {
            pwdx = (char*) 0;
        }
        set_current_wd(pwdx);
    ...
    safe_prompt = ksheuid ? "$ " : "# ";
    {
         ...
         * contain a #.
         */
        if (!(vp->flag & ISSET) || (!ksheuid && !strchr(str_val(vp), '#')))
        {
            /* setstr can't fail here */
    ...
    Flag(FPRIVILEGED) = getuid() != ksheuid || getgid() != getegid() ? 1 : 0;
    ...
    else
    {
        ...
        s->file = "<stdin>";
        s->u.shf = shf_fdopen(0, SHF_RD | can_seek(0), (struct shf*) 0);
    ...
    /* This bizarreness is mandated by POSIX */
    {
        ...
        if (fstat(0, &s_stdin) >= 0 && S_ISCHR(s_stdin.st_mode) && Flag(FTALKING))
        {
            reset_nonblock(0);
        }
    }
    ...
    restricted = (int) Flag(FRESTRICTED);
    Flag(FRESTRICTED) = 0;
    errexit = (int) Flag(FERREXIT);
    Flag(FERREXIT) = 0;
    ...
    else
    {
        ...
        {
            include(substitute("$HOME/kshrc.ksh", 0), 0, (char**) 0, 1);
        }
    }
    ...
    if (restricted)
    {
        static const char* const restr_com[] = {
                "typeset", "-r", "PATH", "ENV", "SHELL", (char*) 0
        };
