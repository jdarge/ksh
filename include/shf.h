...
#define shf_getc(shf)       ((shf)->rnleft > 0 ? (shf)->rnleft--, *(shf)->rp++ : shf_getchar(shf))
#define shf_putc(c, shf)    ((shf)->wnleft == 0 ? shf_putchar((c), (shf)) : ((shf)->wnleft--, *(shf)->wp++ = (c)))
...
struct shf *shf_open ARGS((const char *name, int oflags, int mode, int sflags));

struct shf *shf_fdopen ARGS((int fd, int sflags, struct shf *shf));

struct shf *shf_reopen ARGS((int fd, int sflags, struct shf *shf));

struct shf *shf_sopen ARGS((char *buf, int bsize, int sflags, struct shf *shf));
