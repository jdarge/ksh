...
void tty_init (init_ttystate) int init_ttystate;
{
    ...
    if ((tfd = open(devtty, O_RDWR, 0)) < 0)
    {
        ...
        {
            tty_devtty = 0;
            warningf(false, "No controlling tty (open %s: %s)", devtty, strerror(errno));
        }
    }
    ...
    {
        warningf(false, "j_ttyinit: dup of tty fd failed: %s", strerror(errno));
    }
    else if (fd_clexec(tty_fd) < 0)
    {
        warningf(false, "j_ttyinit: can't set close-on-exec flag: %s", strerror(errno));
        close(tty_fd);
        tty_fd = -1;
    }
