...
void initvar ()
{
    static const struct
    {
        ...
        int v;
            ...
            {"COLUMNS", V_COLUMNS}, {"IFS", V_IFS}, {"OPTIND", V_OPTIND}, {"PATH", V_PATH},
            {"POSIXLY_CORRECT", V_POSIXLY_CORRECT}, {"TMPDIR", V_TMPDIR},
#ifdef HISTORY
            {"HISTFILE", V_HISTFILE}, {"HISTSIZE", V_HISTSIZE},
#endif /* HISTORY */
#ifdef EDIT
            { "EDITOR",		V_EDITOR },
            { "VISUAL",		V_VISUAL },
...
const char* array_index_calc (n, arrayp, valp) const char* n;
                                               bool* arrayp;
                                               int* valp;
{
    const char* p;
 ...
 */
struct tbl* global (n) const char* n;
{
    struct block* l = e->loc;
    struct tbl* vp;
    int c;
...
struct tbl* local (const char* n, bool copy)
{
    ...
    if (copy && !(vp->flag & DEFINED))
    {
        ...
        {
            vp->flag |= vq->flag & (
                    EXPORT | INTEGER | RDONLY | LJUST | RJUST | ZEROFIL | LCASEV | UCASEV_AL |
                    INT_U | INT_L
            );
            if (vq->flag & INTEGER)
            {
...
char* str_val (vp) struct tbl* vp;
{
    ...
    {                /* integer source */
        ...
        const char* digits = (vp->flag & UCASEV_AL) ? "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
int setstr (vq, s, error_ok) struct tbl* vq;
                             const char* s;
                             int error_ok;
{
    char* fs = NULL;
    ...
    { /* string dest */
        ...
        {
            /* debugging */
            if (s >= vq->val.s && s <= vq->val.s + strlen(vq->val.s))
            {
                internal_errorf(true, "setstr: %s=%s: assigning to self", vq->name, s);
            }
...
void setint (vq, n) struct tbl* vq;
                    long n;
{
    if (!(vq->flag & INTEGER))
    {
...
int getint (vp, nump) struct tbl* vp;
                      long* nump;
{
    char* s;
    int c;
 ...
 */
struct tbl* setint_v (vq, vp) struct tbl* vq, * vp;
{
    int base;
    long num;
...
static char* formatstr (vp, s) struct tbl* vp;
                               const char* s;
{
    int olen, nlen;
    char* p, * q;
    ...
    if (vp->flag & (RJUST | LJUST))
    {
        ...
        if (vp->flag & RJUST)
        {
            ...
            shf_snprintf(p, nlen + 1, ((vp->flag & ZEROFIL) && digit(*s)) ? "%0*s%.*s" : "%*s%.*s",
 ...
 */
static void export (vp, val) struct tbl* vp;
                             const char* val;
{
    char* xp;
 ...
 */
struct tbl* typeset (var, set, clr, field, base) const char* var;
                                                 Tflag clr, set;
                                                 int field, base;
{
    struct tbl* vp;
    ...
    if (Flag(FRESTRICTED) && (
            strcmp(tvar, "PATH") == 0 || strcmp(tvar, "ENV") == 0 || strcmp(tvar, "SHELL") == 0
    ))
    {
        errorf("%s: restricted", tvar);
    }

    vp = (set & LOCAL) ? local(tvar, (set & LOCAL_COPY) ? true : false) : global(tvar);
     ...
     * (-L/-R/-Z/-i).
     */
    if ((vpbase->flag & RDONLY) && (val || clr || (set & ~EXPORT)))
    {
        /* XXX check calls - is error here ok by POSIX? */
    ...
    if (set | clr)
    {
        ...
        for (t = vpbase; t; t = t->u.array)
        {
            ...
            if (fake_assign)
            {
                ...
                {
                    s = t->val.s + t->type;
                    free_me = (t->flag & ALLOC) ? t->val.s : (char*) 0;
                }
                t->flag &= ~ALLOC;
            }
    ...
    if ((vpbase->flag & EXPORT) && !(vpbase->flag & INTEGER) && vpbase->type == 0)
    {
 ...
 */
void unset (vp, array_ref) struct tbl* vp;
                           int array_ref;
{
    if (vp->flag & ALLOC)
    {
 ...
 */
char* skip_varname (s, aok) const char* s;
                            int aok;
{
    int alen;
...
char* skip_wdvarname (s, aok) const char* s;
...
int is_wdvarname (s, aok) const char* s;
                          int aok;
{
...
int is_wdvarassign (s) const char* s;
{
...
static int special (name) const char* name;
{
    struct tbl* tp;
...
static void unspecial (name) const char* name;
{
    struct tbl* tp;
...
static void getspec (vp) struct tbl* vp;
{
...
static void setspec (vp) struct tbl* vp;
{
    char* s;
    ...
    switch (special(vp->name))
    {
        ...
        case V_TMPDIR:
             ...
             */
            {
                struct stat statb;
                s = str_val(vp);
                if (ISABSPATH(s) && eaccess(s, W_OK | X_OK) == 0 && stat(s, &statb) == 0 &&
...
static void unsetspec (vp) struct tbl* vp;
{
    switch (special(vp->name))
    {
        case V_PATH:
 ...
 * vp, indexed by val.
 */
static struct tbl* arraysearch (vp, val) struct tbl* vp;
                                         int val;
{
 ...
 * bracket.
 */
int array_ref_len (cp) const char* cp;
{
    const char* s = cp;
    int c;
    int depth = 0;
 ...
 */
char* arrayname (str) const char* str;
{
    const char* p;
 ...
 */
void set_array (var, reset, vals) const char* var;
                                  int reset;
                                  char** vals;
{
    struct tbl* vp, * vq;
