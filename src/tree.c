...
static void ptree (t, indent, shf) struct op* t;
                                   int indent;
                                   struct shf* shf;
{
    char** w;
    ...
    switch (t->type)
    {
        ...
        case TUNTIL:
            ...
            fptreef(shf, indent + 6, "%s %T", (t->type == TWHILE) ? "while" : "until", t->left);
        ...
        case TASYNC:
            ...
            break;
        case TFUNCT:
            fptreef(shf, indent, t->u.ksh_func ? "function %s %T" : "%s() %T", t->str, t->left);
            break;
        case TTIME:
...
static void pioact (shf, indent, iop) struct shf* shf;
                                      int indent;
                                      struct ioword* iop;
{
    int flag = iop->flag;
    ...
    {
        fptreef(shf, indent, (iop->flag & IONAMEXP) ? "%s " : "%S ", iop->name);
    }
}
...
static void tputS (wp, shf) char* wp;
                            struct shf* shf;
{
    int c, quoted = 0;
...
static void vfptreef (shf, indent, fmt, va) struct shf* shf;
                                            int indent;
                                            const char* fmt;
                                            va_list va;
{
...
struct op* tcopy (t, ap) struct op* t;
                         Area* ap;
{
    struct op* r;
    char** tw, ** rw;
    ...
    else
    {
        ...
        rw = r->vars = (char**) alloc((tw - t->vars + 1) * sizeof(*tw), ap);
        for (tw = t->vars; *tw != NULL;)
        {
            *rw++ = wdcopy(*tw++, ap);
        }
    ...
    else
    {
        ...
        rw = r->args = (char**) alloc((tw - t->args + 1) * sizeof(*tw), ap);
        for (tw = t->args; *tw != NULL;)
        {
            *rw++ = wdcopy(*tw++, ap);
        }
...
char* wdcopy (wp, ap) const char* wp;
                      Area* ap;
{
...
char* wdscan (wp, c) const char* wp;
                     int c;
{
    int nest = 0;
...
#endif /* KSH */
            default:
                internal_errorf(0, "wdscan: unknown char 0x%x (carrying on)", wp[-1]);
        }
    }
}
...
static struct ioword** iocopy (iow, ap) struct ioword** iow;
                                        Area* ap;
{
    struct ioword** ior;
    int i;
...
static void iofree (iow, ap) struct ioword** iow;
                             Area* ap;
{
    struct ioword** iop;
    struct ioword* p;
