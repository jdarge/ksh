...
void inittraps ()
{
    ...
    /* Use strsignal for signal descriptions */
    for (i = 0; i < NSIG; i++)
    {
        if (!sigtraps[i].name && strsignal(i) != NULL)
        {
...
Trap* gettrap (name, igncase) const char* name;
                              int igncase;
{
    int i;
    Trap* p;
    ...
    for (p = sigtraps, i = SIGNALS + 1; --i >= 0; p++)
    {
        if (p->name)
        {
            ...
            else
            {
                if (p->name && (
                        !strcmp(p->name, name) || (
                                strlen(name) > 3 && !strncmp("SIG", p->name, 3) &&
...
void runtraps (flag) int flag;
{
    ...
    {
        if (p->set && (
                !flag || ((p->flags & flag) && p->trap == (char*) 0)
        ))
        {
            runtrap(p);
        }
...
void restore_pipe (restore_dfl) int restore_dfl;
{
    if (restore_dfl)
    {
...
void setexecsig (p, restore) Trap* p;
                             int restore;
{
    /* XXX debugging */
    if (!(p->flags & (TF_ORIG_IGN | TF_ORIG_DFL)))
    {
        internal_errorf(1, "setexecsig: unset signal %d(%s)", p->signal, p->name);
    }
