...
static void evalerr ARGS((Expr_state * es, enum error_type type, const char *str)) GCC_FUNC_ATTR(noreturn);
...
static void assign_check ARGS((Expr_state * es, enum token op, struct tbl *vasn));
...
static void evalerr(Expr_state *es, enum error_type type, const char *str) {
    ...
    switch (type) {
        ...
        case ET_RECURSIVE:
            warningf(true, "%s: expression recurses on parameter `%s'", es->expression, str);
            break;

        case ET_LVALUE:
            warningf(true, "%s: %s requires lvalue", es->expression, str);
            break;

        case ET_RDONLY:
            warningf(true, "%s: %s applied to read only variable", es->expression, str);
            break;
...
static struct tbl *evalexpr(Expr_state *es, enum prec prec) {
    ...
    if (prec == P_PRIMARY) {
        op = es->tok;
        if (op == O_BNOT || op == O_LNOT || op == O_MINUS || op == O_PLUS) {
            token(es);
    ...
    for (op = es->tok; IS_BINOP(op) && opinfo[(int) op].prec == prec; op = es->tok) {
        token(es);
        vasn = vl;
        ...
        if ((
                    op == O_DIV || op == O_MOD || op == O_DIVASN || op == O_MODASN
            ) && vr->val.i == 0) {
            if (es->noassign) {
                vr->val.i = 1;
            } else {
