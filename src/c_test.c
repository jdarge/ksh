...
static const struct t_op b_ops[] = {
        ...
        {"==", TO_STEQL},
#endif /* KSH */
        {"!=", TO_STNEQ}, {"<", TO_STLT}, {">", TO_STGT}, {"-eq", TO_INTEQ}, {"-ne", TO_INTNE},
        {"-gt", TO_INTGT}, {"-ge", TO_INTGE}, {"-lt", TO_INTLT}, {"-le", TO_INTLE},
        {"-ef", TO_FILEQ}, {"-nt", TO_FILNT}, {"-ot", TO_FILOT}, {"", TO_NONOP}
};
    ...
    if (argc <= 5)
    {
        ...
        while (--argc >= 0)
        {
            ...
            if (argc == 1)
            {
                ...
                res = (*te.eval)(&te, TO_STNZE, opnd1, (char*) 0, 1);
                if (invert & 1)
                {
                    res = !res;
                }
...
Test_op test_isop (Test_env* te, Test_meta meta, const char* s)
{
    ...
    if (*s)
    {
        ...
        {
            if (sc1 == otab->op_text[1] && strcmp(s, otab->op_text) == 0 && (
...
int test_eval (Test_env* te, Test_op op, const char* opnd1, const char* opnd2, int do_eval)
{
    ...
    switch ((int) op)
    {
        ...
        case TO_INTLT: /* -lt */
        {
            ...
            if (!evaluate(opnd1, &v1, KSH_RETURN_ERROR) || !evaluate(opnd2, &v2, KSH_RETURN_ERROR))
            {
        ...
        case TO_FILNT: /* -nt */
        {
            ...
            return stat(opnd1, &b1) == 0 && (
                    (
                            (s2 = stat(opnd2, &b2)) == 0 && b1.st_mtime > b2.st_mtime
                    ) || s2 < 0
            );
        }
        case TO_FILOT: /* -ot */
        {
            ...
            return stat(opnd2, &b2) == 0 && (
                    (
                            (s1 = stat(opnd1, &b1)) == 0 && b1.st_mtime < b2.st_mtime
                    ) || s1 < 0
            );
        }
        case TO_FILEQ: /* -ef */
            return stat(opnd1, &b1) == 0 && stat(opnd2, &b2) == 0 && b1.st_dev == b2.st_dev &&
                   b1.st_ino == b2.st_ino;
    }
...
static int test_eaccess (const char* pathx, int mode)
{
    ...
    if (res == 0 && ksheuid == 0 && (mode & X_OK))
    {
        ...
        {
            res = (statb.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) ? 0 : -1;
        }
    }
