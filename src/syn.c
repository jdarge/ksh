...
static struct op* pipeline (cf) int cf;
{
...
static struct op* c_list (multi) int multi;
{
    ...
    while (1)
    {
        ...
        {
            p = block(c == '&' ? TASYNC : TCOPROC, p, NOBLOCK, NOWORDS);
        }
        else if (c != ';')
        {
            have_sep = 0;
...
static struct ioword* synio (cf) int cf;
{
    struct ioword* iop;
    int ishere;
...
static struct op* nested (type, smark, emark) int type, smark, emark;
{
    struct op* t;
...
static struct op* get_command (cf) int cf;
{
    struct op* t;
            ...
            while (1)
            {
                ...
                switch (tpeek(cf))
                {
                    ...
                    case LWORD:
                         ...
                         */
                        if (iopn == 0 && XPsize(vars) == 0 && XPsize(args) == 0 &&
    ...
    {
        iops[iopn++] = NULL;
        iops = (struct ioword**) aresize((void*) iops, sizeofN(struct ioword *, iopn), ATEMP);
        t->ioact = iops;
    }
...
static struct op* casepart (endtok) int endtok;
{
    struct op* t;
    int c;
    XPtrV ptns;
...
static struct op* function_body (name, ksh_func) char* name;
...
static struct op* block (type, t1, t2, wp) int type;
                                           struct op* t1, * t2;
                                           char** wp;
{
    struct op* t;
...
const struct tokeninfo
{
    ...
    short reserved;
        ...
        {"select", SELECT, true},
#endif /* KSH */
        {"while", WHILE, true}, {"until", UNTIL, true}, {"do", DO, true}, {"done", DONE, true},
        {"in", IN, true}, {"function", FUNCTION, true}, {"time", TIME, true}, {"{", '{', true},
        {"}", '}', true}, {"!", BANG, true},
#ifdef KSH
        {"[[", DBRACKET, true},
#endif /* KSH */
        /* Lexical tokens (0[EOF], LWORD and REDIR handled specially) */
        {"&&", LOGAND, false}, {"||", LOGOR, false}, {";;", BREAK, false},
#ifdef KSH
        {"((", MDPAREN, false}, {"|&", COPROC, false},
#endif /* KSH */
...
static void syntaxerr (what) const char* what;
{
...
static void nesting_push (save, tok) struct nesting_state* save;
                                     int tok;
{
    *save = nesting;
...
static void nesting_pop (saved) struct nesting_state* saved;
{
    nesting = *saved;
}

static struct op* newtp (type) int type;
{
    struct op* t;
 ...
 *	$
 */
static int assign_command (s) char* s;
{
    char c = *s;
    ...
    return (c == 'a' && strcmp(s, "alias") == 0) || (c == 'e' && strcmp(s, "export") == 0) ||
           (c == 'r' && strcmp(s, "readonly") == 0) || (c == 't' && strcmp(s, "typeset") == 0);
}

/* Check if we are in the middle of reading an alias */
static int inalias (s) struct source* s;
{
...
const char* const dbtest_tokens[] = {
        dbtest_or, dbtest_and, dbtest_not, dbtest_oparen, dbtest_cparen
};
 ...
 */
static int dbtestp_isa (te, meta) Test_env* te;
                                  Test_meta meta;
{
    ...
    else if (meta == TM_UNOP || meta == TM_BINOP)
    {
        ...
        {
            ret = 1;
            save = wdcopy(yylval.iop->flag == IOREAD ? db_lthan : db_gthan, ATEMP);
        }
...
static const char* dbtestp_getopnd (te, op, do_eval) Test_env* te;
                                                     Test_op op;
                                                     int do_eval;
{
...
static int dbtestp_eval (te, op, opnd1, opnd2, do_eval) Test_env* te;
                                                        Test_op op;
                                                        const char* opnd1;
                                                        const char* opnd2;
                                                        int do_eval;
...
static void dbtestp_error (te, offset, msg) Test_env* te;
                                            int offset;
                                            const char* msg;
{
    te->flags |= TEF_ERROR;
    ...
    if (offset < 0)
    {
        ...
        symbol = LWORD;
        yylval.cp = *(
                XPptrv(*te->pos.av) + XPsize(*te->pos.av) + offset
        );
    }
    syntaxerr(msg);
}
