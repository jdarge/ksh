...
void mcheck ()
{
    ...
    if (now - mlastchkd >= mailcheck_interval)
    {
        ...
        while (mbp)
        {
            ...
            {
                if (stbuf.st_size && mbp->mb_mtime != stbuf.st_mtime &&
...
void mpset (mptoparse) char* mptoparse;
{
    mbox_t* mbp;
...
static void munset (mlist) mbox_t* mlist;
{
    mbox_t* mbp;
...
static mbox_t* mballoc (p, m) char* p;
                              char* m;
{
    struct stat stbuf;
    mbox_t* mbp;
...
static void mprintit (mbp) mbox_t* mbp;
{
    struct tbl* vp;
