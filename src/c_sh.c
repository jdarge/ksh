    ...
    for (; *wp != NULL; wp++)
    {
        for (cp = Xstring(cs, cp);;)
        {
            ...
            while (1)
            {
                ...
                if (c == EOF && shf_error(shf) && shf_errno(shf) == EINTR)
                {
        ...
        if (!wp[1])
        {
            while (Xlength(cs, cp) && ctype(cp[-1], C_IFS) && ctype(cp[-1], C_IFSWS))
            {
                cp--;
            }
        }
...
static char* clocktos (t) clock_t t;
{
    ...
    if (CLK_TCK != 100)
    {    /* convert to 1/100'ths */
        t = (t < (clock_t) (1000000000 / CLK_TCK)) ? (t * 100) / CLK_TCK : (t / CLK_TCK) * 100;
    }
