...
int c_fc (wp) char** wp;
{
    ...
    else
    {
         ...
         */
        hfirst = hist_get(first, (lflag || last) ? true : false, lflag ? true : false);
        if (!hfirst)
        {
            return 1;
        }
    ...
    if (lflag)
    {
        ...
        {
            shf_fprintf(shl_stdout, nfmt, hist_source->line - (int) (histptr - hp));
...

    {
        ...
        if (n < 0)
        {
            bi_errorf("error reading temp file %s - %s", tf->name, strerror(shf_errno(shf)));
            shf_close(shf);
            return 1;
        }
        shf_close(shf);
                                  ...
                                  int dowrite;    /* ignored (compatibility with COMPLEX_HISTORY) */
{
    ...
    { /* remove oldest command */
        ...
        memmove(histlist, histlist + 1, sizeof(histlist[0]) * (histsize - 1));
...
void histsave (lno, cmd, dowrite) int lno;
                                  const char* cmd;
                                  int dowrite;
{
