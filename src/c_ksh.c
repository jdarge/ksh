...
#ifdef S_ISLNK
    p = current_wd[0] ? (physical ? get_phys_path(current_wd) : current_wd) : (char*) 0;
#else /* S_ISLNK */
...
#define PO_COPROC    BIT(4)    /* printing to coprocess: block SIGPIPE */
    ...
    else
    {
        ...
        {
            /* treat a lone - like -- */
            if (wp[builtin_opt.optind] && strcmp(wp[builtin_opt.optind], "-") == 0)
            {
                builtin_opt.optind++;
            }
        }
...
int c_whence (char** wp)
{
    ...
    while ((vflag || ret == 0) && (id = *wp++) != NULL)
    {
        ...
        if (vflag || (
                tp->type != CALIAS && tp->type != CEXEC && tp->type != CTALIAS
        ))
        {
            shprintf("%s", id);
        }
        switch (tp->type)
        {
            ...
            case CALIAS:
                if (vflag)
                {
                    shprintf(" is an %salias for ", (tp->flag & EXPORT) ? "exported " : null);
                }
                if (!iam_whence && !vflag)
                {
                    shprintf("alias %s=", id);
            ...
            case CSHELL:
                if (vflag)
                {
                    shprintf(" is a%s shell builtin", (tp->flag & SPEC_BI) ? " special" : null);
                }
                break;
            case CTALIAS:
            case CEXEC:
...
int c_typeset (char** wp)
{
    ...
    if (wp[builtin_opt.optind])
    {
         ...
         */
        if (fset & (
                LJUST | RJUST | ZEROFIL | UCASEV_AL | LCASEV | INTEGER | INT_U | INT_L
        ))
        {
            fclr |= ~fset & (
                    LJUST | RJUST | ZEROFIL | UCASEV_AL | LCASEV | INTEGER | INT_U | INT_L
    ...
    if (wp[builtin_opt.optind])
    {
        ...
        for (i = builtin_opt.optind; wp[i]; i++)
        {
            if (func)
            {
                ...
                {
                    fptreef(shl_stdout, 0, f->flag & FKSH ? "function %s %T\n" : "%s() %T\n", wp[i],
                            f->val.t
                    );
                }
            }
            ...
            for (p = tsort(&l->vars); (vp = *p++);)
            {
                ...
                for (; vp; vp = vp->u.array)
                {
                    ...
                    else
                    {
                        if (pflag)
                        {
                            shprintf("%s ", (flag & EXPORT) ? "export" : "readonly");
                        }
...
int c_alias (char** wp)
{
    ...
    for (; *wp != NULL; wp++)
    {
        ...
        if ((val && !tflag) || (!val && tflag && !Uflag))
        {
            ...
            newval = tflag ? search(alias, path, X_OK, (int*) 0) : val;
            if (newval)
            {
...
static char* kill_fmt_entry (void* arg, int i, char* buf, int buflen)
{
    ...
    if (sigtraps[i].name)
    {
        shf_snprintf(buf, buflen, "%*d %*s %s", ki->num_width, i, ki->name_width, sigtraps[i].name,
                     sigtraps[i].mess
        );
    }
    else
    {
        shf_snprintf(buf, buflen, "%*d %*d %s", ki->num_width, i, ki->name_width,
                     sigtraps[i].signal, sigtraps[i].mess
        );
    }
    return buf;
}
...
int c_kill (char** wp)
{
    ...
    if ((p = wp[1]) && *p == '-' && (digit(p[1]) || isupper((unsigned char) p[1])))
    {
    ...
    {
        shf_fprintf(shl_out, "usage: kill [ -s signame | -signum | -signame ] {pid|job}...\n\
       kill -l [exit_status]\n"
        );
        bi_errorf("%s", null);
        return 1;
    ...
    if (lflag)
    {
        ...
        else
        {
            ...
            print_columns(shl_stdout, SIGNALS - 1, kill_fmt_entry, (void*) &ki,
...
const struct builtin kshbuiltins[] = {
        ...
        {"+cd", c_cd}, {"+command", c_command}, {"echo", c_print}, {"*=export", c_typeset},
#ifdef HISTORY
        {"+fc", c_fc},
#endif /* HISTORY */
        {"+getopts", c_getopts}, {"+jobs", c_jobs}, {"+kill", c_kill},
#ifdef KSH
        {"let", c_let},
#endif /* KSH */
        {"print", c_print}, {"pwd", c_pwd}, {"*=readonly", c_typeset}, {"=typeset", c_typeset},
