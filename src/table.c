...
unsigned int hash (n) const char* n;
{
    unsigned int h = 0;
...
void tinit (tp, ap, tsize) struct table* tp;
                           Area* ap;
                           int tsize;
{
    tp->areap = ap;
...
static void texpand (tp, nsize) struct table* tp;
                                int nsize;
{
    int i;
    struct tbl* tblp, ** p;
    ...
    for (i = 0; i < osize; i++)
    {
        if ((tblp = otblp[i]) != NULL)
        {
            ...
            {
                for (p = &ntblp[hash(tblp->name) & (tp->size - 1)]; *p != NULL; p--)
                {
                    if (p == ntblp)
                    { /* wrap */
                        p += tp->size;
...
struct tbl* mytsearch (tp, n, h) struct table* tp;    /* table */
...
struct tbl* tenter (tp, n, h) struct table* tp;    /* table */
                              const char* n;        /* name to enter */
                              unsigned int h;            /* hash(n) */
{
    struct tbl** pp, * p;
    ...
    len = strlen(n) + 1;
    p = (struct tbl*) alloc(offsetof(struct tbl, name[0]) + len, tp->areap);
    p->flag = 0;
    p->type = 0;
    p->areap = tp->areap;
    p->u2.field = 0;
...
void mytdelete (p) struct tbl* p;
{
    p->flag = 0;
}

void ksh_twalk (ts, tp) struct tstate* ts;
                        struct table* tp;
{
    ts->left = tp->size;
    ts->next = tp->tbls;
...
struct tbl* tnext (ts) struct tstate* ts;
{
    while (--ts->left >= 0)
    {
...
static int tnamecmp (p1, p2) void* p1, * p2;
{
    return strcmp(((struct tbl*) p1)->name, ((struct tbl*) p2)->name);
}

struct tbl** tsort (tp) struct table* tp;
{
    int i;
    struct tbl** p, ** sp, ** dp;
