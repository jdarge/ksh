 ...
 * fails.
 */
struct shf* shf_open (name, oflags, mode, sflags) const char* name;
                                                  int oflags;
                                                  int mode;
                                                  int sflags;
{
...
struct shf* shf_fdopen (fd, sflags, shf) int fd;
                                         int sflags;
                                         struct shf* shf;
{
    int bsize = sflags & SHF_UNBUF ? (sflags & SHF_RD ? 1 : 0) : SHF_BSIZE;
...
struct shf* shf_reopen (fd, sflags, shf) int fd;
                                         int sflags;
                                         struct shf* shf;
{
    int bsize = sflags & SHF_UNBUF ? (sflags & SHF_RD ? 1 : 0) : SHF_BSIZE;
 ...
 */
struct shf* shf_sopen (buf, bsize, sflags, shf) char* buf;
                                                int bsize;
                                                int sflags;
                                                struct shf* shf;
{
...
int shf_close (shf) struct shf* shf;
{
    int ret = 0;
...
int shf_fdclose (shf) struct shf* shf;
{
    int ret = 0;
 ...
 */
char* shf_sclose (shf) struct shf* shf;
{
...
int shf_finish (shf) struct shf* shf;
{
    int ret = 0;
 ...
 */
int shf_flush (shf) struct shf* shf;
{
 ...
 */
static int shf_emptybuf (shf, flags) struct shf* shf;
                                     int flags;
{
    int ret = 0;
    ...
    if (shf->flags & SHF_STRING)
    {
        ...
        nbuf = (unsigned char*) aresize(shf->buf, shf->wbsize * 2, shf->areap);
...
static int shf_fillbuf (shf) struct shf* shf;
{
    if (shf->flags & SHF_STRING)
    {
        return 0;
    ...
    while (1)
    {
        ...
        if (shf->rnleft < 0 && errno == EINTR && !(shf->flags & SHF_INTERRUPT))
        {
            continue;
        }
        break;
 ...
 */
int shf_seek (shf, where, from) struct shf* shf;
                                off_t where;
                                int from;
{
    if (shf->fd < 0)
    ...
    {
        if (from == SEEK_CUR && (
                where < 0 ? -where >= shf->rbsize - shf->rnleft : where < shf->rnleft
        ))
        {
            shf->rnleft -= where;
            shf->rp += where;
 ...
 */
int shf_read (buf, bsize, shf) char* buf;
                               int bsize;
                               struct shf* shf;
{
 ...
 */
char* shf_getse (buf, bsize, shf) char* buf;
                                  int bsize;
                                  struct shf* shf;
{
    unsigned char* end;
...
int shf_getchar (shf) struct shf* shf;
{
    if (!(shf->flags & SHF_RD))
    {
                        ...
                        struct shf* shf;
{
    ...
    if ((shf->flags & SHF_ERROR) || c == EOF || (shf->rp == shf->buf && shf->rnleft))
    {
        return EOF;
    }
 ...
 */
int shf_puts (s, shf) const char* s;
                      struct shf* shf;
{
    if (!s)
    {
...
int shf_write (buf, nbytes, shf) const char* buf;
                                 int nbytes;
                                 struct shf* shf;
{
...
int shf_snprintf (char* buf, int bsize, const char* fmt, ...)
{
    ...
    if (!buf || bsize <= 0)
    {
        internal_errorf(1, "shf_snprintf: buf %lx, bsize %d", (long) buf, bsize);
    }
...
int shf_vfprintf (shf, fmt, args) struct shf* shf;
                                  const char* fmt;
                                  va_list args;
{
    char c, * s;
        ...
        switch (c)
        {
            ...
            case 'x':
                ...
                switch (c)
                {
                    ...
                    case 'x':
                    {
                        const char* digits = (flags & FL_UPPER) ? "0123456789ABCDEF"
                                                                : "0123456789abcdef";
                        do
                        {
                            *--s = digits[lnum & 0xf];
