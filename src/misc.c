 ...
 */
void setctypes (s, t) const char* s;
                      int t;
{
    int i;
...
char* ulton (n, base) unsigned long n;
                      int base;
{
    char* p;
    static char buf[20];
...
char* str_save (s, ap) const char* s;
                       Area* ap;
{
    size_t len;
    char* p;
 ...
 * (unless n < 0).
 */
char* str_nsave (s, n, ap) const char* s;
                           int n;
                           Area* ap;
{
    char* ns;
...
#endif
        {"bgnice", 0, OF_ANY}, {(char*) 0, 'c', OF_CMDLINE},
#ifdef EMACS
        {"emacs", 0, OF_ANY}, {"emacs-usemeta", 0, OF_ANY}, /* non-standard */
#endif
        {"errexit", 'e', OF_ANY},
#ifdef EMACS
        {"gmacs", 0, OF_ANY},
#endif
        {"ignoreeof", 0, OF_ANY}, {"interactive", 'i', OF_CMDLINE}, {"keyword", 'k', OF_ANY},
        {"login", 'l', OF_CMDLINE}, {"markdirs", 'X', OF_ANY},
#ifdef JOBS
        {"monitor", 'm', OF_ANY},
#else /* JOBS */
        { (char *) 0,	'm',		     0 }, /* so FMONITOR not ifdef'd */
#endif /* JOBS */
        {"noclobber", 'C', OF_ANY}, {"noexec", 'n', OF_ANY}, {"noglob", 'f', OF_ANY},
        {"nohup", 0, OF_ANY}, {"nolog", 0, OF_ANY}, /* no effect */
#ifdef    JOBS
        {"notify", 'b', OF_ANY},
#endif    /* JOBS */
        {"nounset", 'u', OF_ANY}, {"physical", 0, OF_ANY}, /* non-standard */
        {"posix", 0, OF_ANY}, /* non-standard */
        {"privileged", 'p', OF_ANY}, {"restricted", 'r', OF_CMDLINE},
        {"stdin", 's', OF_CMDLINE}, /* pseudo non-standard */
        {"trackall", 'h', OF_ANY}, {"verbose", 'v', OF_ANY},
#ifdef VI
        {"vi", 0, OF_ANY}, {"viraw", 0, OF_ANY}, /* no effect */
...
static char* options_fmt_entry (arg, i, buf, buflen) void* arg;
                                                     int i;
                                                     char* buf;
                                                     int buflen;
{
    ...
    shf_snprintf(buf, buflen, "%-*s %s", oi->opt_width, oi->opts[i].name,
                 Flag(oi->opts[i].flag) ? "on" : "off"
    );
    return buf;
}

static void printoptions (verbose) int verbose;
{
    int i;
...
void change_flag (f, what, newval) enum sh_flag f;    /* flag to change */
 ...
 */
int parse_args (argv, what, setargsp) char** argv;
                                      ...
                                      int* setargsp;
{
    ...
    while ((optc = ksh_getopt(argv, &go, opts)) != EOF)
    {
        ...
        switch (optc)
        {
            ...
            default:
                ...
                for (i = 0; i < (int) NELEM(goptions); i++)
                {
                    if (optc == goptions[i].c && (what & goptions[i].flags))
                    {
                        change_flag((enum sh_flag) i, what, set);
                        break;
                    }
    ...
    if (!(go.info & GI_MINUSMINUS) && argv[go.optind] &&
        (argv[go.optind][0] == '-' || argv[go.optind][0] == '+') && argv[go.optind][1] == '\0')
    {
    ...
    if (setargsp)
    {
        ...
        *setargsp = !arrayset && (
                (go.info & GI_MINUSMINUS) || argv[go.optind]
        );
    }
...
int bi_getn (as, ai) const char* as;
                     int* ai;
{
    int rv = getn(as, ai);
...
int gmatch (s, p, isfile) const char* s, * p;
                          int isfile;
{
    const char* se, * pe;
    ...
    if (!isfile && !has_globbing(p, pe))
    {
        int len = pe - p + 1;
        char tbuf[64];
        char* t = len <= (int) sizeof(tbuf) ? tbuf : (char*) alloc(len, ATEMP);
        debunk(t, p, len);
        return !strcmp(t, s);
    }
    return do_gmatch((const unsigned char*) s, (const unsigned char*) se, (const unsigned char*) p,
                     (const unsigned char*) pe, isfile
    );
}
...
- return ?
*/
int has_globbing (xp, xpe) const char* xp, * xpe;
{
...
static int do_gmatch (s, se, p, pe, isfile) const unsigned char* s, * p;
    ...
    while (p < pe || s < se)
    {
        ...
        switch (pc)
        {
            ...
            case 0x80 | '*': /* matches zero or more times */
                ...
                if (p[-1] == (0x80 | '*') && do_gmatch(s, se, prest, pe, isfile))
                {
                    return 1;
                }
                for (psub = p;; psub = pnext)
                {
                    ...
                    {
                        if (do_gmatch(s, srest, psub, pnext - 2, isfile) && (
                                do_gmatch(srest, se, prest, pe, isfile) || (
                                        s != srest && do_gmatch(srest, se, p - 2, pe, isfile)
                                )
                        ))
                        {
                            return 1;
            ...
            case 0x80 | '!': /* matches none of the patterns */
                ...
                for (srest = s; srest <= se; srest++)
                {
                    ...
                    if (!matched && do_gmatch(srest, se, prest, pe, isfile))
                    {
                        return 1;
                    }
                }
...
static const unsigned char* cclass (p, sub) const unsigned char* p;
                                            int sub;
{
    int c, d, not, found = 0;
...
const unsigned char* pat_scan (p, pe, match_sep) const unsigned char* p;
...
void qsortp (base, n, f) void** base;                /* base address */
                         size_t n;                /* elements */
...
static void qsort1 (base, lim, f) void** base, ** lim;
...
int xstrcmp (p1, p2) void* p1, * p2;
{
    return (strcmp((char*) p1, (char*) p2));
}

/* Initialize a Getopt structure */
void ksh_getopt_reset (go, flags) Getopt* go;
                                  int flags;
{
    go->optind = 1;
    go->optarg = (char*) 0;
                                   ...
                                   const char* options;
{
    ...
    if (c == '?' || c == ':' || c == ';' || c == ',' || c == '#' || !(o = strchr(options, c)))
    {
        ...
        {
            warningf(true, "%s%s-%c: unknown option", (go->flags & GF_NONAME) ? "" : argv[0],
    ...
    if (*++o == ':' || *o == ';')
    {
        ...
        else
        {
            ...
            warningf(true, "%s%s-`%c' requires argument", (go->flags & GF_NONAME) ? "" : argv[0],
                     (go->flags & GF_NONAME) ? "" : ": ", c
            );
            if (go->flags & GF_ERROR)
            {
 ...
 */
void print_value_quoted (s) const char* s;
{
    const char* p;
    int inquote = 0;
 ...
 * element
 */
void print_columns (shf, n, func, arg, max_width, prefcol) struct shf* shf;
                                                           int n;
                                                           char* (* func) ARGS(
                                                           (void *, int, char *, int));
                                                           void* arg;
                                                           ...
                                                           int prefcol;
{
    ...
    for (r = 0; r < rows; r++)
    {
        for (c = 0; c < cols; c++)
        {
            ...
            if (i < n)
            {
                shf_fprintf(shf, "%-*s", max_width, (*func)(arg, i, str, max_width + 1));
                if (c + 1 < cols)
                {
...
int strip_nuls (buf, nbytes) char* buf;
                             int nbytes;
{
    char* dst;
 ...
 * Returns dst.
 */
char* str_zcpy (dst, src, dsize) char* dst;
                                 const char* src;
                                 int dsize;
{
    if (dsize > 0)
 ...
 * and restarts read.
 */
int blocking_read (fd, buf, nbytes) int fd;
                                    char* buf;
                                    int nbytes;
{
    int ret;
...
char* ksh_get_wd (buf, bsize) char* buf;
                              int bsize;
{
#ifdef HAVE_GETCWD
    char* b;
